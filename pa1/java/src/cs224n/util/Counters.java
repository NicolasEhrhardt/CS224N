...
public class Counters {
  ...
  public static <E> void normalizeInPlace(Counter<E> counter) {
    double total = counter.totalCount();
    for (E key : counter.keySet()) {
      counter.setCount(key, counter.getCount(key) / total);
    }
  }
  ...
  public static <K, V> CounterMap<K, V> conditionalNormalizeInPlace(CounterMap<K,V> counterMap) {
    for (K key : counterMap.keySet()) {
      normalizeInPlace(counterMap.getCounter(key));
    }
    return counterMap;
  }
  ...
  public static <A, B> double epsilon(CounterMap<A, B> c1, CounterMap<A, B> c2) {
        double tot = 0.;
        for (A key: c1.keySet()) {
            for (B val: c1.getCounter(key).keySet()) {
                double diff = c1.getCount(key, val) - c2.getCount(key, val);

                tot += diff*diff;
            }
        }
        tot /= (double) c1.totalSize();
        return tot;
  }
}
